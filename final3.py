...
with st.sidebar:
    ...
    density_threshold_slider = st.slider("Set Density Threshold Multiplier", min_value=1.0, max_value=3.0, value=1.5, step=0.1)
    ...
    while cap.isOpened():
        ...
        if frame_count % frame_skip == 0:
            ...
            if len(x_coords) > 0:
                ...
                hexbin_plot = plt.hexbin(x_coords, height - np.array(y_coords), gridsize=6, mincnt=1, vmin=0, vmax=10, cmap='Blues')
                ...
                if len(density_history) > 5:  # Arbitrary threshold for stability
                    ...
                    if current_density > threshold:
                        hexbin_alert_placeholder.warning(f"Alert: ⚠️ Density exceeded threshold! Current Density: {current_density:.2f} objects/hexagon")
