...
if uploaded_video is not None:
    ...
    # Layout to position video and hexbin side by side
    video_col, hexbin_col = st.columns([1, 1])
    video_placeholder = video_col.empty()
    hexbin_chart_placeholder = hexbin_col.empty()

    # Place scatter plot and summary metrics below the video and hexbin chart
    scatterplot_col, summary_metrics_col = st.columns([1, 1])
    scatterplot_placeholder = scatterplot_col.empty()
    summary_metrics_placeholder = summary_metrics_col.empty()
...
while cap.isOpened():
    ...
    if frame_count % frame_skip == 0:
        ...
        # Update Hexbin plot for density
        if len(x_coords) > 0:
        ...
        if len(x_coords) > 0:
            ...
            plt.grid(True)
            plt.legend()
            scatterplot_placeholder.pyplot(plt)
